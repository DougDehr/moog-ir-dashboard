...
# ---------------------------------------------------------------------------
# Two-tier cache: st.cache_data applies the SAME ttl to a failed/empty
# result as a successful one. Every fetch function below used a long ttl
# (12h in most cases) so results stay put across pages/reruns — but that
# means a single transient Yahoo/SEC failure gets cached and replayed for
# the FULL ttl, even after the upstream service recovers a minute later.
# That's what turned one rate-limit blip into a reported ~16-hour outage in
# the deployed app, far longer than Yahoo's own typical (minutes-to-a-few-
# hours) clearing pattern. _cached() below keeps the long ttl for real data
# but gives empty/failed results a short one (a few minutes), so the app
# retries again soon instead of being stuck.
# ---------------------------------------------------------------------------

@st.cache_resource(show_spinner=False)
def _memo_store() -> dict:
    """Shared, process-wide store backing _cached() — persists across
    reruns/sessions like st.cache_data's own store does, but as a plain
    dict we control directly so we can apply a different ttl to failures."""
    return {}


def _safe_copy(value):
    """st.cache_data pickles/copies every return value automatically, so a
    caller mutating what it gets back can't corrupt the cached copy for
    everyone else. Our manual store hands out the same object reference by
    default, so it needs to do that defensively itself."""
...
def _cached(cache_key: tuple, fetch_fn, success_ttl: float, error_ttl: float = 180, is_empty=None):
    if is_empty is None:
    ...
    if entry is not None:
        value, ts, was_empty = entry
        if now - ts < (error_ttl if was_empty else success_ttl):
            return _safe_copy(value)
    value = fetch_fn()
    store[cache_key] = (value, now, is_empty(value))
    return _safe_copy(value)
...
def get_price_history(tickers: tuple[str, ...], period: str = "5y", interval: str = "1d") -> pd.DataFrame:
    ...
    return _cached(
        ("price_history", tickers, period, interval), _fetch,
        success_ttl=60 * 45, is_empty=lambda v: v is None or v.empty,
    )
...
def get_quote(ticker: str) -> dict:
    ...
    return _cached(
        ("quote", ticker), _fetch,
        success_ttl=60 * 20, is_empty=lambda v: "error" in v,
    )
...
def get_fundamentals(ticker: str) -> dict:
    ...
    return _cached(
        ("fundamentals", ticker), _fetch,
        success_ttl=60 * 60 * 12, is_empty=lambda v: "error" in v,
    )
...
def get_recommendations_trend(ticker: str) -> pd.DataFrame:
    ...
    return _cached(("recommendations_trend", ticker), _fetch, success_ttl=60 * 60 * 12, is_empty=lambda v: v.empty)
...
def get_upgrades_downgrades(ticker: str, limit: int = 25) -> pd.DataFrame:
    ...
    def _fetch():
        ...
        if df is None or df.empty:
            return pd.DataFrame()
        return df.reset_index().sort_values("GradeDate", ascending=False).head(limit)

    return _cached(("upgrades_downgrades", ticker, limit), _fetch, success_ttl=60 * 60 * 12, is_empty=lambda v: v.empty)
...
def get_major_holders_breakdown(ticker: str) -> dict:
    ...
    return _cached(("major_holders", ticker), _fetch, success_ttl=60 * 60 * 12)
...
def get_institutional_holders(ticker: str, limit: int = 10) -> pd.DataFrame:
    ...
    def _fetch():
        try:
            ...
            return pd.DataFrame()
        return df.head(limit) if df is not None and not df.empty else pd.DataFrame()

    return _cached(("institutional_holders", ticker, limit), _fetch, success_ttl=60 * 60 * 12, is_empty=lambda v: v.empty)
...
def get_insider_transactions(ticker: str, limit: int = 25) -> pd.DataFrame:
    ...
    return _cached(("insider_transactions", ticker, limit), _fetch, success_ttl=60 * 60 * 12, is_empty=lambda v: v.empty)
...
def get_earnings_history(ticker: str, limit: int = 12) -> pd.DataFrame:
    ...
    return _cached(("earnings_history", ticker, limit), _fetch, success_ttl=60 * 60 * 12, is_empty=lambda v: v.empty)
...
def get_eps_trend(ticker: str) -> pd.DataFrame:
    ...
    return _cached(("eps_trend", ticker), _fetch, success_ttl=60 * 60 * 12, is_empty=lambda v: v.empty)
...
def get_dividends(ticker: str, period: str = "5y") -> pd.Series:
    ...
    return _cached(("dividends", ticker, period), _fetch, success_ttl=60 * 60 * 12, is_empty=lambda v: v.empty)
...
def get_company_tickers_map() -> dict:
    ...
    return _cached(("company_tickers_map",), _fetch, success_ttl=60 * 60 * 12)
...
def get_recent_filings(cik: str, forms: tuple[str, ...] = ("10-K", "10-Q", "8-K"), limit: int = 12) -> pd.DataFrame:
    ...
    return _cached(("recent_filings", cik, forms, limit), _fetch, success_ttl=60 * 60 * 6, is_empty=lambda v: v.empty)
...
def get_concept_disclosure_status(cik: str, tag: str, taxonomy: str = "us-gaap") -> str:
    ...
    # "not_tagged" is a real, stable answer (cache it long-term same as
    # "has_data") — only "fetch_failed" is the transient case that should
    # retry soon.
    return _cached(
        ("concept_disclosure_status", cik, tag, taxonomy), _fetch,
        success_ttl=60 * 60 * 12, is_empty=lambda v: v == "fetch_failed",
    )
...
def get_xbrl_concept(cik: str, tag: str, taxonomy: str = "us-gaap") -> pd.DataFrame:
    ...
    return _cached(("xbrl_concept", cik, tag, taxonomy), _fetch, success_ttl=60 * 60 * 12, is_empty=lambda v: v.empty)
...
def get_financial_series(cik: str, concept_key: str) -> pd.DataFrame:
    ...
    return _cached(("financial_series", cik, concept_key), _fetch, success_ttl=60 * 60 * 12, is_empty=lambda v: v.empty)
